...
import {
  getSavedPublicUserFromLocalStorage,
  validateEmail,
} from "../../../utils/utils";
    ...
    switch (nextPage) {
      ...
      case 3:
        if (!cameraPermissionGranted)
          return toast.info(
            "Please grant access to your audio and video before proceeding"
          );
      ...
      case 4: {
        ...
        if (!locationAccessGranted)
          return toast.info(
            "Please grant access to your location before proceeding"
          );
        ...
        const allSavedEventDetailsForUser =
          getSavedPublicUserFromLocalStorage();

        const updatedEventsForUser =
          allSavedEventDetailsForUser &&
          Array.isArray(allSavedEventDetailsForUser)
            ? allSavedEventDetailsForUser
            : [];
        ...
        try {
          ...
          localStorage.setItem(
            PUBLIC_USER_DETAIL_KEY_IN_LOCAL_STORAGE,
            JSON.stringify(updatedEventsForUser)
          );
          ...
          if (error?.response?.status === 409) {
            const eventDetailForUserIsAlreadySaved = updatedEventsForUser.find(
              (item) => item.event_id === searchParams.get("event_id")
            );
      ...
      >
        <div className={styles.event__Wrapper}>
          <div className={styles.event__Live__Info}>
            ...
            <p>
              <span>
                {`${countDownTimer?.days < 10 ? "0" : ""}${
                  countDownTimer?.days
                }`}
                days{" "}
                {`${countDownTimer?.hours < 10 ? "0" : ""}${
                  countDownTimer?.hours
                }`}
                hours{" "}
                {`${countDownTimer?.minutes < 10 ? "0" : ""}${
                  countDownTimer?.minutes
                }`}
                minutes{" "}
                {`${countDownTimer?.seconds < 10 ? "0" : ""}${
    ...
    <>
      <div className={styles.wrapper}>
        <div className={styles.event__Card}>
          <img src={dowellLogo} alt="dowell logo" className={styles.logo} />
            ...
            <>
              ...
              <div className={styles.event__Details}>
                <p style={{ textAlign: "center" }}>
                  <span className={styles.info__Bold}>
                    We cannot seem to find this event, it has either been
                    deleted or it does not exist
                  </span>
                </p>
              </div>
            </>
          ) : new Date().getTime() >
            new Date(foundEventDetail?.close_date).getTime() ? (
            <>
            ...
            <>
              ...
              <div className={styles.event__Details}>
                {currentFormPage === 0 ? (
                  <>
                    <p>
                      <span className={styles.info__Bold}>Starts: </span>
                      {new Date(
                        foundEventDetail?.start_time
                      ).toDateString()} by{" "}
                      {new Date(
                        foundEventDetail?.start_time
                      ).toLocaleTimeString()}
                    ...
                    <p>
                      <span className={styles.info__Bold}>Ends: </span>
                      {new Date(
                        foundEventDetail?.close_date
                      ).toDateString()} by{" "}
                      {new Date(
                        foundEventDetail?.close_date
                      ).toLocaleTimeString()}
                    ...
                    <br />
                    <p>
                      <span className={styles.info__Bold}>Time allowed: </span>
                      {isNaN(foundEventDetail?.duration_in_hours)
                        ? foundEventDetail?.duration_in_hours
                        : Number(
                            foundEventDetail?.duration_in_hours
                          ).toLocaleString()}{" "}
                      hour{foundEventDetail?.duration_in_hours > 1 ? "s" : ""}
                    </p>
                  </>
                ) : currentFormPage === 1 ? (
                  <div className={styles.user__Detail}>
                    <label>
                      ...
                      <input
                        ...
                        onChange={({ target }) =>
                          handleUpdateUserDetail(target.name, target.value)
                        }
                      />
                    </label>
                    <label>
                  ...
                  <div className={styles.user__Detail}>
                    <video
                      ...
                      className={`${styles.video__Item} ${
                        !cameraPermissionGranted ? styles.no__Display : ""
                      }`}
                    ></video>
                    {cameraPermissionGranted ? (
                      <>
                        <p>
                          Please ensure your face is fully visible in the frame
                      ...
                      <>
                        <p>
                          Please grant access to your video and audio by
                  ...
                  <>
                    {locationAccessGranted ? (
                      ...
                      <>
                        <p>
                          One last step. Please grant location access by
                          clicking the button below
                        </p>
                        <button
                          style={{
